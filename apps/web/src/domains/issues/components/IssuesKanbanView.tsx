...
export function IssuesKanbanView({ boardColumns, recentUnblocks }: IssuesKanbanViewProps) {
	...
	const defaultTeamId = useMemo(() => defaultTeamIdFromGroups(groups), [groups])
	const openNewIssue = useCallback(() => setNewDialogOpen(true), [])
...
function defaultTeamIdFromGroups(groups: Record<IssueState, LaunchQueueItem[]>): string | null {
	let latest: { teamId: string; updatedAt: number } | null = null
	for (const state of ISSUE_STATE_ORDER) {
		for (const item of groups[state]) {
			if (item.kind !== 'issue' || item.issue.team_id === null) continue
			const updatedAt = Date.parse(item.issue.updated_at)
			if (!latest || updatedAt > latest.updatedAt) {
				latest = { teamId: item.issue.team_id, updatedAt }
			}
		}
	}
	return latest?.teamId ?? null
}
