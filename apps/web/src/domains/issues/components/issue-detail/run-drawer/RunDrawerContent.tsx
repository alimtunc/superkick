...
import { RunDrawerTabs } from '@/domains/issues/components/issue-detail/run-drawer/RunDrawerTabs'
...
export function RunDrawerContent({ runId }: RunDrawerContentProps) {
	...
	return (
		<RunSessionGate
			session={detail}
			errorTitle="Run load failed"
			notFoundMessage="It may have been deleted or the identifier is wrong."
		>
			{({ run, pr, sessions, attentionRequests, isTerminal, events }) => (
