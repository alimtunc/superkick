import { RuntimeStatusBadge } from '@/components/app/badges'
import { NoProvidersDetected } from '@/domains/settings/components/NoProvidersDetected'
import { ProviderRow } from '@/domains/settings/components/ProviderRow'
	...
	return (
		<article className="rounded-[7px] border border-border bg-raised shadow-(--shadow-sm)">
			<header className="flex flex-wrap items-center gap-3 border-b border-border px-4 py-3">
				...
				{runtime.host_label ? (
					<span className="font-data text-[10px] text-fg-muted">{runtime.host_label}</span>
				) : null}
				{platform ? <span className="font-data text-[10px] text-fg-dim">{platform}</span> : null}
				{runtime.last_seen_at ? (
					<span className="font-data ml-auto text-[10px] text-fg-dim">
