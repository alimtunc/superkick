...
import { fmtElapsed, watchButtonClass } from "./utils";
...
export function AlertRow({ run, refTime, reason, isLast }: {
  ...
  return (
    <div className={`flex items-center gap-3 px-4 py-2.5 hover:bg-oxide-dim/40 transition-colors ${borderClass} group`}>
      <span className={`status-bar h-6 ${isBlocked ? "bg-oxide live-pulse" : "bg-gold"}`} />
      <Link to={`/runs/${run.id}`} className="flex items-center gap-3 min-w-0 flex-1">
      ...
      <button
        ...
        className={`shrink-0 font-data text-[10px] px-1 transition-colors ${watchButtonClass(watched, maxReached)}`}
