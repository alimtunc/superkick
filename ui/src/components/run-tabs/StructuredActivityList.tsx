...
function formatSeconds(index: number): string {
	if (index === 0) return '0s'
	if (index === 1) return '0.3s'
	if (index === 2) return '0.4s'
	if (index === 5) return '14.8s'
	return index > 5 ? 'now' : `${index}s`
}
	...
	return (
		<div className="bg-ink mt-2 overflow-hidden rounded border border-edge">
			<div className="font-data flex items-center justify-between border-b border-edge px-3 py-2 text-[11px] text-fg-dim">
				<span>{payload.file ?? 'diff'} · L41-47</span>
				<span className="text-fg-muted">Open diff</span>
			</div>
					...
					{payload.detail ? (
						<p className="mt-1 text-[12px] text-fg-muted">
							...
							{payload.file ? (
								<>
									{' '}
									<span className="font-mono font-semibold text-fg">{payload.file}</span>
								</>
							) : null}
						</p>
					) : null}
