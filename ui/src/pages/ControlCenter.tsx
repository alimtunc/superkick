  ...
  return (
    <>
      ...
      <div className="mx-auto flex max-w-360 flex-col gap-16 px-6 py-12">
        {d.error ? <div className="panel glow-red font-data p-3 text-[12px] text-oxide">{d.error}</div> : null}
        ...
        <div className="flex flex-col gap-6">
          <div className="fade-up grid grid-cols-2 gap-4 md:grid-cols-4">
            ...
            <MetricCard
              ...
              sub={d.needsAttention.length === 0 ? "all clear" : `${d.waitingHuman.length} blocked · ${d.failed.length} failed`}
            ...
            <MetricCard
              ...
              color={d.successRate !== null && d.successRate >= 80 ? "mineral" : d.successRate !== null && d.successRate < 50 ? "oxide" : "silver"}
            />
          </div>
          ...
          <div className="fade-up grid grid-cols-3 gap-3 delay-1 md:grid-cols-6">
            ...
            <KpiCell label="Waiting" value={d.waitingHuman.length} alert={d.waitingHuman.length > 0} />
        ...
        {d.needsAttention.length > 0 || d.aging.length > 0 ? (
          <section className="fade-up delay-2">
            <SectionTitle title="ATTENTION" accent="oxide" count={d.needsAttention.length + d.aging.length} />
            <div className="panel glow-red overflow-hidden">
              ...
              {d.aging.map((run, i) => (
                <AlertRow key={run.id} run={run} refTime={d.refTime} reason={`Aging — ${d.oldestActive} elapsed`} isLast={i === d.aging.length - 1} />
              ))}
            </div>
          </section>
        ) : null}
        ...
        {d.active.length > 0 ? (
          <section className="fade-up delay-3">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              <BoardCol title="In Progress" count={d.inProgress.length} runs={d.inProgress} refTime={d.refTime} accent="cyan" />
              <BoardCol title="Needs Human" count={d.waitingHuman.length} runs={d.waitingHuman} refTime={d.refTime} accent="gold" />
              <BoardCol title="Queued" count={d.queued.length} runs={d.queued} refTime={d.refTime} accent="dim" />
            </div>
          </section>
        ) : null}
        ...
        <section className="fade-up">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div className="panel p-4">
              <h4 className="font-data mb-4 text-[10px] tracking-wider text-dim uppercase">Avg Duration</h4>
              <div className="space-y-3">
                <DurationRow label="Completed" value={avgDuration(d.completed)} color="text-mineral" />
                <DurationRow label="Failed" value={avgDuration(d.failed)} color="text-oxide" />
                <DurationRow label="Median (all)" value={medianDuration(d.runs)} color="text-silver" />
              </div>
            </div>
          </div>
        </section>
